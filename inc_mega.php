...
/* CUSTOM FUNCTIONS */
function cbc_mac_file($filename,$k,$n){
	$fileSize = filesize($filename);
	$fp = fopen($filename,'r');

	$padding_size = ($fileSize % 16) == 0 ? 0 : 16 - $fileSize % 16;
...
function sendFile($uploadUrl,$fileName){
	$CR = "\r\n";
	...
	// Url and port
	$uinfo = parse_url(trim($uploadUrl));
	$ssl = false;$port = 80;if($uinfo['scheme'] == 'https'){$ssl = true;$port=443;}

	// Open socket
	$fp = fsockopen(($ssl ? 'ssl://' : '' ).$uinfo['host'],$port,$errno,$errstr,30);
	if(!$fp){return false;}
	
	// Header
	$header = 'POST '.(isset($uinfo['path']) ? $uinfo['path'] : '/').((isset($uinfo['query']) && !empty($uinfo['query'])) ? '?'.$uinfo['query'] : '').' HTTP/1.1'.$CR.
	'Host: '.$uinfo['host'].$CR.
	'Accept: */*'.$CR.
	'Content-Length: '.$fileSize.$CR.
	'Content-Type: application/x-www-form-urlencoded'.$CR.
	'Expect: 100-continue'.$CR.$CR;


	$bytes = 0;$write = substr($header,$bytes,8192);$bytes += 8192;
	while(fwrite($fp,$write) === false){$write = substr($header,$bytes,8192);$bytes += 8192;}
	$bytesUploaded = 0;
	while($block = fread($file,8192)){
		$bytesUploaded += strlen($block);
		showStatus($bytesUploaded,$fileSize,130);
		fwrite($fp,$block);
	}
	fwrite($fp,$CR.$CR);
